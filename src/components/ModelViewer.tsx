  ...
  React.useEffect(() => {
    const loadModel = async () => {
      try {
                ...
                fbxLoader.load(
                  ...
                  (error) => {
                    ...
                    if (error.message && error.message.includes('version not supported')) {
                      reject(
                        new Error(
                          'FBX file version not supported. Please try a different FBX file or convert to glTF format.'
                        )
                      );
                    } else {
        ...
        loadedModel.scale.setScalar(autoScale);
        loadedModel.position.multiplyScalar(autoScale);
      ...
      <div className="flex-1 bg-gray-900">
        <Canvas camera={{ position: [5, 5, 5], fov: 50 }} shadows>
          <Suspense fallback={<Loader />}>
            ...
            {/* Controls */}
            <OrbitControls enablePan={true} enableZoom={true} enableRotate={true} minDistance={1} maxDistance={50} />
          </Suspense>
        </Canvas>
      </div>
    </div>
